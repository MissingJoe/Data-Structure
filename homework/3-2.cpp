...
void add(LinkList &b, LinkList c) {
	LinkList q, p;
	LinkList a;
	initial(&a);
	int loc = 1;
...
void multiply_1(LinkList& a, Node *b, LinkList c) {
	LinkList p = b, q = c->next;
	int loc = 1;
	while (q) {
		insert(&a, p->data_ei + q->data_ei, p->data_pi * q->data_pi, loc++);
		q = q->next;
	}
}

Status multiply_all(LinkList& a, LinkList b, LinkList c,int m) {
	LinkList p = b->next;
	LinkList* l = (LinkList*)malloc(sizeof(LinkList) * (m));
	if (!l)
		return -2;
	int i;
	for (i = 0; i < m; i++)
...
int main() {
	...
	LinkList test3;
	initial(&test3);
	multiply_all(test3, test1, test2, length1);
